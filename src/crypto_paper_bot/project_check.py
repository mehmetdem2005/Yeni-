...
def main() -> int:
    ...
    if (FRONTEND / "node_modules").exists():
        checks.append(run(["npm", "run", "typecheck"], cwd=FRONTEND, timeout=180))
        checks.append(run(["npm", "run", "build"], cwd=FRONTEND, timeout=240))
    else:
        checks.append(CheckResult("npm run typecheck", True, "Atlandı: frontend/node_modules yok. Önce cd frontend && npm install çalıştır."))
